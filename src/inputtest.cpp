...
void clickDownEvent (const Event* e, void *data) {
    ...
    if (mouseWatcher->has_mouse()){
        ...
        if (entry) {
            ...
            hit_pos = entry->get_surface_point(window->get_render());
...
void set_events () {
    framework.define_key ( "control-mouse1", "Mouse 1 Action", &clickDownEvent, NULL );
	framework.define_key ( "mouse1-up", "Mouse 1 Up Action", &clickUpEvent, NULL );
